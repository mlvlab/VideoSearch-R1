...
LOG_VISION_DETAILS = os.getenv("QWENVL_LOG_VISION", "0") == "1"
...
def _read_video_npy(
    ...
    """Read video from a pre-extracted npy/npz file.

    Supported shapes:
      - (T, H, W, C)
      - (T, C, H, W)
      - (T, H, W) -> treated as grayscale
      - (H, W, C) -> single frame
    """
    video_path = ele["video"]
    if video_path.startswith("file://"):
        video_path = video_path[7:]
    st = time.time()
    arr = np.load(video_path, allow_pickle=False)
    ...
    if arr.ndim == 2:
        ...
        raise ValueError(f"Unsupported npy shape {arr.shape} for {video_path}")

    if arr.shape[1] in (1, 3, 4) and arr.shape[-1] not in (1, 3, 4):
        arr_tchw = arr
    else:
        arr_tchw = arr.transpose(0, 3, 1, 2)

    if arr_tchw.shape[1] == 1:
        arr_tchw = np.repeat(arr_tchw, 3, axis=1)
    elif arr_tchw.shape[1] > 3:
        arr_tchw = arr_tchw[:, :3]

    video = torch.from_numpy(arr_tchw)
    if video.dtype == torch.float64:
        video = video.float()

    total_frames = int(video.shape[0])
    sampled_frames = int(total_frames)
    nframes = ceil_by_factor(total_frames, FRAME_FACTOR)
    if total_frames < nframes:
        pad = video[-1:].repeat(nframes - total_frames, 1, 1, 1)
        video = torch.cat([video, pad], dim=0)
        total_frames = int(video.shape[0])
    sample_fps = float(ele.get("fps", FPS))
    raw_fps = float(ele.get("raw_fps", sample_fps))
    if raw_fps <= 0:
        raw_fps = sample_fps

    frames_indices = ele.get(
        "frames_indices", ele.get("sampled_indices_src", None)
    )
    if isinstance(frames_indices, (list, tuple)):
        parsed_indices = []
        for item in frames_indices:
            try:
        ...
        if not parsed_indices:
            parsed_indices = [i for i in range(sampled_frames)]
    else:
        parsed_indices = [i for i in range(sampled_frames)]

    if len(parsed_indices) < total_frames:
        parsed_indices.extend([parsed_indices[-1]] * (total_frames - len(parsed_indices)))
    elif len(parsed_indices) > total_frames:
        parsed_indices = parsed_indices[:total_frames]

    total_num_frames = ele.get("total_num_frames", None)
    try:
        total_num_frames = int(total_num_frames) if total_num_frames is not None else None
    except Exception:
        total_num_frames = None
    if total_num_frames is None or total_num_frames <= 0:
        if parsed_indices:
            total_num_frames = int(max(parsed_indices) + 1)
        else:
            total_num_frames = int(total_frames)

    video_metadata = dict(
        fps=raw_fps,
        frames_indices=parsed_indices,
        total_num_frames=total_num_frames,
        video_backend="npy",
    )
    if LOG_VISION_DETAILS:
        logger.info(
            f"npy:  {video_path=}, {total_frames=}, load time={time.time() - st:.3f}s"
        )
    else:
        logger.debug(
            f"npy:  {video_path=}, {total_frames=}, load time={time.time() - st:.3f}s"
...
def fetch_video(
    ...
    if isinstance(ele["video"], str):
        if ele["video"].endswith(".npy") or ele["video"].endswith(".npz"):
            video, video_metadata, sample_fps = _read_video_npy(ele)
        else:
