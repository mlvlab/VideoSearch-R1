...
SHUFFLE="${SHUFFLE:-0}"
SEED="${SEED:-42}"
SHARD_GPUS="${SHARD_GPUS:-"2,3"}"          # e.g. "0,1,2,3" => one local_vllm process per GPU
...
if [[ -z "${SHARD_GPUS}" ]]; then
  ...
  echo "[dataset_generation][didemo] generated: ${OUTPUT_JSONL}"
  exit 0
fi

if [[ "${BACKEND}" != "local_vllm" ]]; then
  echo "[dataset_generation][didemo][error] SHARD_GPUS mode requires BACKEND=local_vllm." >&2
  exit 1
fi
if [[ -n "${JOBS_JSONL}" ]]; then
  echo "[dataset_generation][didemo][error] SHARD_GPUS mode does not support JOBS_JSONL yet." >&2
  exit 1
fi
...
if (( NUM_SHARDS < 1 )); then
  echo "[dataset_generation][didemo][error] SHARD_GPUS is set but no valid GPU id was parsed." >&2
  exit 1
fi

if [[ "${LOCAL_TENSOR_PARALLEL_SIZE}" != "1" ]]; then
  echo "[dataset_generation][didemo][warn] SHARD_GPUS mode forces LOCAL_TENSOR_PARALLEL_SIZE=1 (current=${LOCAL_TENSOR_PARALLEL_SIZE})."
fi
