...
if [[ -z "${SHARD_GPUS}" ]]; then
  ...
  echo "[dataset_generation][charades] generated: ${OUTPUT_JSONL}"
  exit 0
fi

if [[ "${BACKEND}" != "local_vllm" ]]; then
  echo "[dataset_generation][charades][error] SHARD_GPUS mode requires BACKEND=local_vllm." >&2
  exit 1
fi
if [[ -n "${JOBS_JSONL}" ]]; then
  echo "[dataset_generation][charades][error] SHARD_GPUS mode does not support JOBS_JSONL yet." >&2
  exit 1
fi
...
if (( NUM_SHARDS < 1 )); then
  echo "[dataset_generation][charades][error] SHARD_GPUS is set but no valid GPU id was parsed." >&2
  exit 1
fi

if [[ "${LOCAL_TENSOR_PARALLEL_SIZE}" != "1" ]]; then
  echo "[dataset_generation][charades][warn] SHARD_GPUS mode forces LOCAL_TENSOR_PARALLEL_SIZE=1 (current=${LOCAL_TENSOR_PARALLEL_SIZE})."
fi
