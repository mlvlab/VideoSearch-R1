...
class Qwen3VLEmbedder():
    ...
    def format_model_input(
        ...
        if video:
            ...
            if isinstance(video, list):
                ...
                if resized_height is not None and int(resized_height) > 0:
                    video_kwargs["resized_height"] = int(resized_height)
                if resized_width is not None and int(resized_width) > 0:
                    video_kwargs["resized_width"] = int(resized_width)
                if video_start is not None:
                    video_kwargs["video_start"] = float(video_start)
                if video_end is not None:
                    video_kwargs["video_end"] = float(video_end)
            else:
    ...
    def process(self, inputs: List[Dict[str, Any]], normalize: bool = True) -> tuple:
        conversations = [self.format_model_input(
            ...
            resized_height=ele.get('resized_height'),
            resized_width=ele.get('resized_width'),
