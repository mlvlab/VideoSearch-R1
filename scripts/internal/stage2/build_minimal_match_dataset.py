...
def _iter_json_rows(path: str) -> Iterable[dict]:
    # Supports both jsonl and json(list)
...
def _load_query_meta(query_meta_path: str) -> Dict[str, Tuple[int, str, str]]:
    # query -> (row_idx_in_query_embeddings, qid, pos_doc_id)
...
SFT_STYLE_PROBLEM_PREFIX = (
    "You are a video retrieval assistant.\n"
    "Analyze the retrieved video against the query.\n"
    "First, write concise reasoning inside <think>...</think>.\n"
    "Then output exactly one final decision as either:\n"
    "<answer>matched</answer> <REFINE>\n"
    "or\n"
    "<answer>not_matched</answer> <REFINE>\n"
    "In both cases, <REFINE> must be appended at the very end.\n"
    "Do not invent details beyond what is visible in the video.\n"
)


def build_top1_dataset(
    verified_jsonl: str,
    ...
    query_key: str = "fig_desc",
    gt_key: str = "video",
    time_key: str = "time",
    duration_key: str = "duration",
    hard_negative_topk: int = 24,
    hard_negative_depth: int = 200,
    augment_match_to_balance: bool = False,
    target_match_ratio: float = 1.0,
    augment_seed: int = 42,
) -> None:
    ...
    for ex in _iter_json_rows(verified_jsonl):
        stats["rows_total"] += 1
        ...
        problem = (
            f"{SFT_STYLE_PROBLEM_PREFIX}\n"
            "User query:\n"
            f"{query}"
        )
        row = {
            "problem_id": -1,
            "qid": qid or f"q_{q_row}",
            "problem": problem,
            "data_type": "video",
            ...
            "data_source": "Verified-Search-Minimal-Top1",
            "query": query,
            "gt_video": gt_video,
            "gt_time": gt_time,
    ...
    rows_not_matched_raw = sum(1 for r in rows if str(r.get("label", "")).strip() == "not_matched")
    ...
    rows_not_matched_final = sum(1 for r in rows if str(r.get("label", "")).strip() == "not_matched")
    ...
    print(
        "[build_minimal_match_dataset] "
        f"wrote={stats['rows_written']} "
        f"matched={stats['rows_matched']} "
        f"not_matched={stats['rows_not_matched']} "
        f"-> {output_jsonl}"
    )


def main():
    data_root = os.environ.get("VIDEOSEARCH_DATA_ROOT", "data")
    activitynet_root = os.path.join(data_root, "activitynet")
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--verified_jsonl",
        type=str,
        default=os.path.join(activitynet_root, "raw_annotation/train.jsonl"),
    )
    parser.add_argument(
        "--query_embeddings_path",
        type=str,
        default=os.path.join(activitynet_root, "train/query_embedding/query_embeddings.train.npy"),
    )
    parser.add_argument(
        "--query_meta_path",
        type=str,
        default=os.path.join(activitynet_root, "train/query_embedding/query_meta.train.jsonl"),
    )
    parser.add_argument(
        "--index_faiss_path",
        type=str,
        default=os.path.join(activitynet_root, "train/index/index.faiss"),
    )
    parser.add_argument(
        "--index_id_map_path",
        type=str,
        default=os.path.join(activitynet_root, "train/index/id_map.json"),
    )
    parser.add_argument(
        "--video_root",
        type=str,
        default=os.path.join(activitynet_root, "train/video_npy_with_meta"),
    )
    parser.add_argument(
        "--output_jsonl",
        type=str,
        default=os.path.join(activitynet_root, "grpo_data/train.minimal_top1.jsonl"),
    )
    parser.add_argument(
        "--output_stats_json",
        type=str,
        default=os.path.join(activitynet_root, "grpo_data/stats.minimal_top1.json"),
    )
    parser.add_argument("--query_key", type=str, default="fig_desc")
    parser.add_argument("--gt_key", type=str, default="video")
    parser.add_argument("--time_key", type=str, default="time")
    parser.add_argument("--duration_key", type=str, default="duration")
    parser.add_argument("--hard_negative_topk", type=int, default=24)
    parser.add_argument("--hard_negative_depth", type=int, default=200)
    parser.add_argument("--augment_match_to_balance", type=int, default=1)
    parser.add_argument("--target_match_ratio", type=float, default=1.0)
