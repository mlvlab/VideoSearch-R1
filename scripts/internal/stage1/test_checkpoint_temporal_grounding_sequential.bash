...
# EVAL_GPUS=1,2 NUM_PROCESSES_PER_GPU=1,1 USE_VLLM=True VLLM_GPU_MEMORY_UTILIZATION=0.65 VLLM_MAX_NUM_SEQS=8 VLLM_EVAL_BATCH_SIZE=8 VLLM_ENFORCE_EAGER=True bash ./scripts/inference/inference.bash didemo --sequential

usage() {
  cat <<'EOF'
Usage:
  bash test_checkpoint_temporal_grounding_sequential.bash /path/to/checkpoint-XXX
  CHECKPOINT_PATH=/path/to/checkpoint-XXX bash test_checkpoint_temporal_grounding_sequential.bash
  DEFAULT_CHECKPOINT=didemo-stage2 bash test_checkpoint_temporal_grounding_sequential.bash
  bash test_checkpoint_temporal_grounding_sequential.bash /path/to/checkpoint-XXX --use_updated_query
...
Examples:
  MAX_TURN=2 bash test_checkpoint_temporal_grounding_sequential.bash /hub_data2/.../checkpoint-115
  EVAL_GPU=2 MAX_TURN=3 RESUME_FROM_JSONL=True bash test_checkpoint_temporal_grounding_sequential.bash /hub_data2/.../checkpoint-115
  bash test_checkpoint_temporal_grounding_sequential.bash /hub_data2/.../checkpoint-115 --use_updated_query
EOF
}
...
eval_script="${repo_root}/scripts/internal/stage1/eval_verified_test_rerank_temporal_grounding_sequential.py"
...
if [[ -d "${checkpoint_query_embedder_dir}" ]]; then
  ...
  echo "[test_checkpoint_temporal_grounding_sequential] detected tuned query embedder: ${query_embedder_model_path}"
else
  echo "[test_checkpoint_temporal_grounding_sequential] query embedder path: ${query_embedder_model_path}"
fi

echo "[test_checkpoint_temporal_grounding_sequential] checkpoint=${checkpoint_path}"
echo "[test_checkpoint_temporal_grounding_sequential] logs_dir=${logs_dir}"
echo "[test_checkpoint_temporal_grounding_sequential] eval_gpus=${eval_gpus} num_processes_per_gpu=${num_processes_per_gpu} total_workers=${total_workers}"
echo "[test_checkpoint_temporal_grounding_sequential] reranks=${reranks} resume_from_jsonl=${resume_from_jsonl}"
echo "[test_checkpoint_temporal_grounding_sequential] temporal=${temporal} max_turn=${max_turn} iou_thresholds=${iou_thresholds}"
echo "[test_checkpoint_temporal_grounding_sequential] progress_interval_sec=${progress_interval_sec}"
echo "[test_checkpoint_temporal_grounding_sequential] use_tqdm=${use_tqdm} tqdm_mininterval=${tqdm_mininterval}"
echo "[test_checkpoint_temporal_grounding_sequential] use_vllm=${use_vllm} tp=${vllm_tensor_parallel_size} mem_util=${vllm_gpu_memory_utilization} max_num_seqs=${vllm_max_num_seqs}"
echo "[test_checkpoint_temporal_grounding_sequential] vllm_eval_batch_size=${vllm_eval_batch_size}"
echo "[test_checkpoint_temporal_grounding_sequential] vllm_model_path=${vllm_model_path:-auto(_vllm_compat)}"
echo "[test_checkpoint_temporal_grounding_sequential] use_updated_query=${use_updated_query}"
echo "[test_checkpoint_temporal_grounding_sequential] query_embedder_input_prefix=${query_embedder_input_prefix:-none}"
echo "[test_checkpoint_temporal_grounding_sequential] video_meta_path=${video_meta_path:-auto}"
...
run_eval_worker() {
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" PYTHONUNBUFFERED=1 "${python_bin}" "${eval_script}" \
...
PY
  echo "[test_checkpoint_temporal_grounding_sequential] ${tag} merged jsonl: ${base_jsonl} (sources=${#merge_sources[@]}, check=${check_jsonl})"
}
...
for raw_rk in "${RK_ARR[@]}"; do
  ...
  output_json="${logs_dir}/external_verified_test_temporal_grounding_sequential_${ckpt_name}${suffix}.json"
  output_jsonl="${logs_dir}/external_verified_test_temporal_grounding_sequential_${ckpt_name}${suffix}.jsonl"

  echo "[test_checkpoint_temporal_grounding_sequential] rerank_topk=${rk} ->"
  ...
  echo "[test_checkpoint_temporal_grounding_sequential] launching ${total_workers} workers (sharded)"
  pids=()
  worker_shard_id=0
  ...
  if [[ "${failed}" -ne 0 ]]; then
    echo "[test_checkpoint_temporal_grounding_sequential] one or more shard workers failed (partial progress merged)." >&2
    exit 1
  fi
done
